...
def _write_master_xlsx(all_papers, out_folder, search_terms=None, title=''):
    ...
    try:
        ...
        for i, (country, flag) in enumerate(countries_list, 4):
            ...
            region = "North America" if country in ["United States","Canada","Mexico","Brazil"] else                      "Europe" if country in ["UK","Germany","France","Spain"] else                      "Asia" if country in ["China","Japan","Korea","India"] else                      "Middle East" if country in ["Saudi Arabia","Egypt","Turkey"] else                      "Africa" if country in ["Libya","Tunisia","Morocco","Algeria","Nigeria","South Africa"] else                      "Oceania"
        ...
        for i, (t, cnt) in enumerate(sorted(tw.items(), key=lambda x: -x[1])[:30], 4):
            ...
            cat = "Education" if any(w in t for w in ["learning","teaching","education","student","teacher"]) else                   "Technology" if any(w in t for w in ["digital","mobile","technology","online","computer","AI"]) else                   "Language" if any(w in t for w in ["language","vocabulary","speaking","writing","EFL","ESL"]) else                   "Methodology" if any(w in t for w in ["qualitative","quantitative","mixed","method"]) else "Education"
